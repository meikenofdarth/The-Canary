...
_LOCATION_PREP = re.compile(
    r"\b(?:in|at|for|around|near|of)\s+([A-Z][a-zA-Z\s]{2,24})",
    re.IGNORECASE,
)
...
def _extract_weather_entities(text: str) -> dict:
    ...
    time_m = _TIME_REFS.search(text)
    if time_m:
        entities["time_reference"] = time_m.group(0).lower()

    loc_m = _LOCATION_PREP.search(text)
    if loc_m:
        candidate = loc_m.group(1).strip().rstrip(".?,!;")
        # Keep only consecutive capitalized words
        ...
        for w in words:
            if w[0].isupper():
                ...
                break
        candidate = " ".join(loc_words)
        # Reject very generic words that are not place names
        generic = {"music", "news", "songs", "song", "weather", "forecast", "here", "home",
                   "the", "a", "an", "me", "my", "us", "this", "that"}
...
def _extract_news_entities(text: str) -> dict:
    ...
    topics = _NEWS_TOPICS.findall(text)
    if topics:
        entities["topics"] = list(dict.fromkeys(t.lower() for t in topics))

    src_m = _NEWS_SOURCES.search(text)
    if src_m:
